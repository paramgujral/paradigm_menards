...
test.describe('Garage Popular Doors Tests', () => {
    ...
    test('"Start Designing" takes this user to Garage Popular Doors Sizes/Styles ', async ({ page }) => {
    ...
    test('Garage-doors-Popular-test-Verifying the Current Tab', async ({ page }) => {
        //await loginInWindows(page);
        // navigate to the starting point with a new quote
        await startQuote(page); // Contains URL verification/assertion, no assertion needed here.
        await navigateToMostPopularSizesStylesPage(page);
    ...
    test('Garage-doors-Popular-test-Availability of the SKU items', async ({ page }) => {
        //await loginInWindows(page);
        // navigate to the starting point with a new quote
        await startQuote(page); // Contains URL verification/assertion, no assertion needed here.
    ...
    test('Garage-doors-Popular-test-Pagination ', async ({ page }) => {
        //await loginInWindows(page);
        // navigate to the starting point with a new quote
        await startQuote(page); // Contains URL verification/assertion, no assertion needed here.
    ...
    test('Garage-doors-Popular-test-Pagination with only one page ', async ({ page }) => {
        // navigate to the starting point with a new quote
        await startQuote(page); // Contains URL verification/assertion, no assertion needed here.
        await navigateToMostPopularSizesStylesPage(page);
    ...
    test('Garage-doors-Popular-test-SKU Number Search ', async ({ page }) => {
        //await loginInWindows(page);
        // navigate to the starting point with a new quote
        await startQuote(page); // Contains URL verification/assertion, no assertion needed here.
        await navigateToMostPopularSizesStylesPage(page);
    ...
    test('Garage-doors-Popular-test-SKU Number Search with no data  ', async ({ page }) => {
        //await loginInWindows(page);
        // navigate to the starting point with a new quote
        await startQuote(page); // Contains URL verification/assertion, no assertion needed here.
    ...
    test('Garage-doors-Popular-test-SKU Description Search ', async ({ page }) => {
        //await loginInWindows(page);
        // navigate to the starting point with a new quote
        await startQuote(page); // Contains URL verification/assertion, no assertion needed here.
    ...
    test('Garage-doors-Popular-test-Enter Both SKU Number and SKU Description then Search', async ({ page }) => {
        //await loginInWindows(page);
        // navigate to the starting point with a new quote
        await startQuote(page); // Contains URL verification/assertion, no assertion needed here.
    ...
    test('Garage-doors-Popular-test-SKU Search Without Entering Anything  ', async ({ page }) => {
        //await loginInWindows(page);
        // navigate to the starting point with a new quote
        await startQuote(page); // Contains URL verification/assertion, no assertion needed here.
    ...
    test('Garage-doors-Popular-test-Edit Quanitity of a SKU Item', async ({ page }) => {
        //await loginInWindows(page);
        // navigate to the starting point with a new quote
        await startQuote(page); // Contains URL verification/assertion, no assertion needed here.
        await navigateToMostPopularSizesStylesPage(page);
    ...
    //Summary
    test('Garage-doors-Popular-test-Verify Summary Click functionality without adding any item ', async ({ page }) => {
        await startQuote(page); // Contains URL verification/assertion, no assertion needed here.
        await summaryTestsSpecs.verifyInaccessibilityOfSummaryTab(page)
    });

    test('Garage-doors-Popular Design",  click on rename Button   ', async ({ page }) => {
    ...
    test('Garage-doors-Popular-test- Check the line items in the design	  ', async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
    ...
    // test.only('Garage-doors-Popular-test-Verify the Room field on a line item		  ', async ({ page }) => {
    //     await startQuote(page);
    //     await navigateToMostPopularSizesStylesPage(page);
    //     test.setTimeout(2 * 60 * 2000);
    //     await page.waitForSelector('#spinner', { state: 'hidden' });
    ...
    // test.only('Garage-doors-Popular-test-The Room should be carried over from the configuration	  ', async ({ page }) => {
    //     await startQuote(page);
    //     await navigateToMostPopularSizesStylesPage(page);
    //     test.setTimeout(2 * 60 * 2000);
    //     await page.waitForSelector('#spinner', { state: 'hidden' });
    ...
    test('Garage-doors-Popular-test- Check line description		  ', async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
        test.setTimeout(2 * 60 * 2000);
        await page.waitForSelector('#spinner', { state: 'hidden' });
    ...
    test('Garage-doors-Popular-test- Check Qty and Price	', async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
        await summaryTestsSpecs.qtyPriceVerification(page)
    });

    test('Garage-doors-Popular-test-Edit/Copy/Delete the line item ', async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
    ...
    test('Garage-doors-Popular-test-verify Print Design button', async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
        test.setTimeout(2 * 60 * 2000);
        await page.waitForSelector('#spinner', { state: 'hidden' });
    ...
    test('Garage-doors-Popular-test-verify Copy Design button', async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
        test.setTimeout(2 * 60 * 2000);
        await page.waitForSelector('#spinner', { state: 'hidden' });
    ...
    test('Garage-doors-Popular-test-verify Restart Design button	', async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
        test.setTimeout(2 * 60 * 2000);
        await page.waitForSelector('#spinner', { state: 'hidden' });
    ...
    test('Garage-doors-Popular-test-Verifying-Online purchase Tab', async ({ page }) => {
        ...
        await purchaseTests.verifyAddToCartDisclamier(page);
    })

    //global change

    test('Garage-doors-Popular-test-If there is only one garage door line in the design, global changes should not be triggered', async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
        test.setTimeout(2 * 60 * 2000);
        await page.waitForSelector('#spinner', { state: 'hidden' });
        ...
        // Click text=Almond
        await page.locator('text=Almond').click();
    ...
    test("Garage-doors-Popular-test-If there're more than one garage door line from a same catalog, but the Q&A you edited is not checked off with Include in Global Changes in the Question template, global changes should not be triggered", async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
        test.setTimeout(2 * 60 * 2000);
        await page.waitForSelector('#spinner', { state: 'hidden' });
    ...
    test("Garage-doors-Popular-test-If there're more than one doors line from a same catalog, but the Q&A you edited is checked off with Include in Global Changes in the Question template, global changes should be triggered", async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
        test.setTimeout(2 * 60 * 2000);
        await page.waitForSelector('#spinner', { state: 'hidden' });
        ...
        await expect.soft(newlyAddedItem).toHaveText(/Color = Almond/)

    })

    test("Garage-doors-Popular-test-It should take the user to the global changes screen asking user if s/he wants to apply this change to other garage-doors on the design", async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
        test.setTimeout(2 * 60 * 2000);
        await page.waitForSelector('#spinner', { state: 'hidden' });
        ...
        await page.locator('button:has-text("Design Options")').click();
        await page.locator('text=Almond').click();
        await page.locator('text=Traditional - Short').click();
        await page.locator('button:has-text("Save Line")').first().click()
        await globalChange.verifyGlobalChangeIsTriggered(page)
        ...
        // let newlyAddedItem2 = page.locator('div:nth-child(2) > .panel-body:has-text("Traditional - Short")')
        let newlyAddedItem1 = page.locator('text=Almond')
        let newlyAddedItem2 = page.locator('text=Traditional - Short')
        ...
        await page.locator('text=Apply Changes').click();
        await page.locator('text=More Specs >> nth=1').click();
        await expect.soft(newlyAddedItem2).toHaveText(/Traditional - Short/)
    })


    test("Garage-doors-Popular-test-Apply Global Changes ", async ({ page }) => {
        await startQuote(page);
        await navigateToMostPopularSizesStylesPage(page);
        test.setTimeout(2 * 60 * 2000);
        await page.waitForSelector('#spinner', { state: 'hidden' });
        ...
        await expect.soft(lockHardwareType).toHaveText("Traditional - Short")
        await page.locator('text=Cancel').click();
        await page.waitForTimeout(2000)
